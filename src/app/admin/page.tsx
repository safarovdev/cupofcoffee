...
export default function AdminPage() {
  ...
  const handleMigrate = async () => {
    ...
    try {
      ...
      await batch.commit();
      toast({ title: 'Успех', description: 'База данных заполнена.' });
    } catch (error) {
      toast({ variant: 'destructive', title: 'Ошибка', description: 'Доступ запрещен.' });
    } finally {
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-4 sm:p-6">
        <Card className="w-full max-w-[380px] p-6 sm:p-8 space-y-6 rounded-[2rem] border-none shadow-xl bg-card">
          <CardHeader className="p-0 text-center space-y-2">
            ...
            <CardTitle className="text-xl font-bold uppercase tracking-tight">Вход в панель</CardTitle>
            <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest">AromaFlow Admin</p>
          </CardHeader>
    ...
    return (
      <div className="h-screen flex items-center justify-center p-4">
        <Card className="max-w-md p-8 text-center space-y-4 rounded-3xl border-2 border-destructive/10 shadow-lg bg-card">
          ...
          <p className="text-muted-foreground text-sm">Аккаунт <strong>{user.email}</strong> не является администратором.</p>
          <div className="bg-muted p-3 rounded-xl text-[10px] font-mono select-all">UID: {user.uid}</div>
      ...
      <main className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-accent/5 p-6 rounded-3xl border border-accent/10">
          <div>
            ...
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Активных позиций: {menuItems?.length || 0}</p>
          </div>
          <Button onClick={handleMigrate} disabled={isMigrating} size="sm" className="rounded-xl h-10 px-5 gap-2">
            <Database className="w-4 h-4" /> {isMigrating ? 'Загрузка...' : 'Восстановить меню'}
          </Button>
        </div>
        ...
        <Tabs defaultValue="items" className="w-full">
          <TabsList className="grid w-full grid-cols-2 rounded-2xl p-1 bg-muted h-12">
            ...
            <TabsTrigger value="add" className="rounded-xl font-bold text-xs">Добавить</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="items" className="mt-6 space-y-3">
            {menuLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {menuItems?.map(item => (
                  <div key={item.id} className="flex items-center justify-between p-4 rounded-2xl bg-card border shadow-sm group">
            ...
            <Card className="p-6 sm:p-8 rounded-3xl border shadow-lg bg-card">
              <form className="space-y-5" onSubmit={(e) => {
                ...
                if (firestore) {
                  ...
                  toast({ title: 'Добавлено', description: 'Товар создан.' });
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input name="category" placeholder="coffee, mojito..." required className="rounded-xl h-11 bg-muted/50 border-none" />
                  </div>
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input name="ingredients" className="rounded-xl h-11 bg-muted/50 border-none" />
                </div>
